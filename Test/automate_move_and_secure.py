...
ADMIN_ROLE_ID = "1475604973952696403"
...
def main():
    print("--- Starting Move and Secure Process ---")
    
    # 1. Secure the System Category
    ...
    for i, chan in enumerate(work_channels):
        chan_id = chan['id']
        chan_name = chan['name']
        print(f"Moving '{chan_name}' ({chan_id}) to System...")
        
        # When moving to a category, we should also try to sync permissions
        # PATCH /channels/{id} with parent_id
        move_payload = {
